...
# ==========================================================
# FRONT-ARC OBSTACLE SCAN  ← NEW
#
# Checks how many / how close obstacles are inside a forward
# cone of half-angle FRONT_HALF_ANGLE centred on the robot's
# current yaw.  Returns:
#   front_blocked  : bool   — at least one obstacle inside the
#                             cone AND closer than FRONT_BLOCK_DIST
#   closest_front  : float  — distance to nearest front obstacle
#                             (inf if none)
#   clear_side     : +1 or -1  — which lateral side has more
#                             room (+1 = robot's left, -1 = right)
# ==========================================================
...
def scan_front_arc(robot_pos, robot_yaw, obstacle_positions):
    ...
    for obs in obstacle_positions:
        ...
        # signed lateral offset (positive = robot's left)
        ...
        if angle < FRONT_HALF_ANGLE and d < FRONT_BLOCK_DIST:
            ...
            # accumulate inverse-distance weight on each side
            if lateral >= 0:
...
# ==========================================================
# LYAPUNOV CONTROLLER  (reverse-aware)
# ==========================================================
...
def lyapunov_controller(current_pos, current_vel, current_yaw,
                        ...
                        closest_front=float('inf')):
    """
    When front_blocked is True the controller:
      • Reverses the robot (negative forward speed).
      • Steers hard toward clear_side so the robot arcs away
        from the obstacle cluster while reversing.
      • Reduces Kp contribution so the attractive goal force
        does not fight the reverse manoeuvre.

    Lyapunov consistency
    --------------------
    During a reverse phase the barrier term h(x) is still
    evaluated and fed into u_barrier, which steers laterally.
    The reverse speed is a short-term override that decreases
    the Lyapunov candidate V by moving the robot away from the
    obstacle (increasing h) while the barrier + steering terms
    already guarantee V̇ < 0 in the tangential direction.
    """
    K_theta     = 2.0
    K_b         = 3.5
    ROBOT_RADIUS= 0.65
    ...
    # ── Desired heading ─────────────────────────────────────────────
    if front_blocked:
        # Steer strongly toward the clearer side while reversing.
        # clear_side: +1 = robot's left, -1 = robot's right.
        # We add a large lateral bias so theta_des points sideways.
    ...
    # ── Speed decision ──────────────────────────────────────────────
    if front_blocked:
        # How aggressively to reverse scales with how close the
        # nearest front obstacle is.  Max reverse at STOP_DIST.
...
def main():
    ...
    initial_obs = [
    # 🔴 FRONT BLOCK (forces detour)
    [2.5,  0.2, 0.5],
    [2.5, -0.2, 0.5],

    # 🟡 SLIGHT OFFSET (creates asymmetry → clear_side matters)
    [3.5,  0.8, 0.5],
    [3.8, -0.6, 0.5],

    # 🟢 FINAL CLUSTER (tests stability near goal)
    [4.8,  0.6, 0.5],
    [5.2, -0.4, 0.5],]
    ...
    for i in range(5000):
        ...
        if GAIN_NET is not None:
            Kp_nn, Kd_nn = map(float, GAIN_NET.forward(img_features))
        else:
            Kp_nn, Kd_nn = FALLBACK_Kp, FALLBACK_Kd

        # ================= BASE MODULATION =================
        danger_stop, min_dist, _, prox_factor = check_proximity(
        ...
        # Near goal
        dist_to_goal_est = np.linalg.norm(np.array(est_pos) - np.array(target_pos[:2]))
        if dist_to_goal_est < 1.0:
            Kp *= 0.5
            Kd *= 1.5
        if dist_to_goal_est < 0.3:
        ...
        # Side bias
        lateral_bias = 0.0
        for obs in current_obs_positions:
            vec = np.array(obs[:2]) - np.array(gt_pos[:2])
            d = np.linalg.norm(vec)

            if d < 1.5 and d > 1e-3:
                heading = np.array([np.cos(yaw), np.sin(yaw)])
                cross = heading[0]*vec[1] - heading[1]*vec[0]
        ...
        # ================= CONTROLLER =================
        if not estop_active:
            v, omega, _, theta_des = lyapunov_controller(
            ...
            goal_vec = np.array(target_pos[:2]) - np.array(gt_pos[:2])
            goal_dir = goal_vec / (np.linalg.norm(goal_vec) + 1e-6)

            robot_fwd = np.array([np.cos(yaw), np.sin(yaw)])

            alignment = np.dot(goal_dir, robot_fwd)

            detour_needed = front_blocked and alignment > 0.7

            # Apply existing lateral bias
            theta_des += lateral_bias

            # Apply smart detour (ONLY when needed)
            if detour_needed:
                theta_des += clear_side * 0.8   # ~45° turn (realistic, not sideways)
